        ...
        try:
            ...
            for key in ("risk_level", "risk_reason", "report_category", "report_text"):
                if key not in result:
                    result[key] = ""
            # P2-5: 校验 risk_level 合法性，防止模型返回变体表述
            if result["risk_level"] not in _VALID_LEVELS:
                result["risk_level"] = "分析失败"
            return result
        except json.JSONDecodeError as e:
            # P2-6: JSON 解析失败不重试，模型输出已损坏，重试大概率也失败
            return {
                "risk_level": "分析失败",
                "risk_reason": f"JSON解析失败: {e}",
                "report_category": "",
                "report_text": "",
            }
        except Exception as e:
            if attempt < retry:
                # P2-6: Rate Limit 指数退避；其他网络错误固定等 2 秒
                try:
                    from openai import RateLimitError
                    wait = min(2 ** (attempt + 2), 30) if isinstance(e, RateLimitError) else 2
                except ImportError:
                    wait = 2
                print(f"  [重试 {attempt + 1}/{retry}] {e}，等待{wait}秒")
                time.sleep(wait)
            else:
                return {
                    "risk_level": "分析失败",
